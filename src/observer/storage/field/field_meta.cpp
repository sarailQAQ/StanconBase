...
const static Json::StaticString FIELD_IS_NULLABLE("nullable");

FieldMeta::FieldMeta() : attr_type_(AttrType::UNDEFINED), attr_offset_(-1), attr_len_(0), visible_(false),is_nullable_(false)
{}

FieldMeta::FieldMeta(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible,bool is_nullable)
{
  [[maybe_unused]] RC rc = this->init(name, attr_type, attr_offset, attr_len, visible,is_nullable);
  ASSERT(rc == RC::SUCCESS, "failed to init field meta. rc=%s", strrc(rc));
}

RC FieldMeta::init(const char *name, AttrType attr_type, int attr_offset, int attr_len, bool visible,bool is_nullable)
{
...
void FieldMeta::desc(std::ostream &os) const
{
  os << "field name=" << name_ << ", type=" << attr_type_to_string(attr_type_) << ", len=" << attr_len_
       << ", visible=" << (visible_ ? "yes" : "no") << ", nullable=" << (is_nullable_ ? "yes" : "no");
}
...
void FieldMeta::to_json(Json::Value &json_value) const
{
  ...
  json_value[FIELD_IS_NULLABLE] = is_nullable_;
}
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field)
{
  ...
  const Json::Value &is_nullable = json_value[FIELD_IS_NULLABLE];
  ...
  bool nullable = is_nullable.asBool();
