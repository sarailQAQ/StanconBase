...
RC MvccTrxKit::init()
{
  fields_ = vector<FieldMeta>{
    FieldMeta("__trx_xid_begin", AttrType::INTS, 0/*attr_offset*/, 4/*attr_len*/, false/*visible*/,true),
    FieldMeta("__trx_xid_end",   AttrType::INTS, 0/*attr_offset*/, 4/*attr_len*/, false/*visible*/,true)
  };
...
RC MvccTrx::update_record(Table *table, Record &record, std::vector<const FieldMeta* >fieldMeta, std::vector<int> idxs, std::vector<Value> &values) {
  ...
  return insert_record(table, old_rec);
}
